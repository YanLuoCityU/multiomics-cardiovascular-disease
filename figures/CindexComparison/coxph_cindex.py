...
class CoxPH():
    ...
    def nested_cross_validation(self):
        ...
        output_dir = os.path.join(self.results_dir, 'Cindex/All')
    ...
    def _process_fold(self, outcome, fold):
        ...
        prefix = 'train'
        X_train = self.__load_predictors(prefix, outcome, fold)
        ...
        prefix = 'test'
        X_test = self.__load_predictors(prefix, outcome, fold)
    ...
    def __load_predictors(self, prefix, outcome, fold):
        ...
        if 'Creatinine' in self.predictor_set:
            creatinine = pd.read_feather(os.path.join(self.split_seed_dir + f"X_{prefix}_Metabolomics_K{fold}.feather"))
            creatinine = creatinine[['eid'] + ['Creatinine']]
            X = creatinine if X.empty else pd.merge(X, creatinine, on='eid', how='left')
            
        if 'Albumin' in self.predictor_set:
            albumin = pd.read_feather(os.path.join(self.split_seed_dir + f"X_{prefix}_Metabolomics_K{fold}.feather"))
            albumin = albumin[['eid'] + ['Albumin']]
            X = albumin if X.empty else pd.merge(X, albumin, on='eid', how='left')
            
        if 'Gly' in self.predictor_set:
            gly = pd.read_feather(os.path.join(self.split_seed_dir + f"X_{prefix}_Metabolomics_K{fold}.feather"))
            gly = gly[['eid'] + ['Gly']]
            X = gly if X.empty else pd.merge(X, gly, on='eid', how='left')

        if 'GlycA' in self.predictor_set:
            glyca = pd.read_feather(os.path.join(self.split_seed_dir + f"X_{prefix}_Metabolomics_K{fold}.feather"))
            glyca = glyca[['eid'] + ['GlycA']]
            X = glyca if X.empty else pd.merge(X, glyca, on='eid', how='left')
        ...
        if 'NPPB' in self.predictor_set:
            nppb = pd.read_feather(os.path.join(self.split_seed_dir + f"X_{prefix}_Proteomics_K{fold}.feather"))
            nppb = nppb[['eid'] + ['NPPB']]
            X = nppb if X.empty else pd.merge(X, nppb, on='eid', how='left')
...
if __name__ == '__main__':
    ...
    for predictor_set in predictor_sets:
        log_filename = os.path.join(log_dir, f"Model/CoxPH/All/{'_'.join(predictor_set)}.log")
        logger = setup_logger(log_filename)
        
        # Log the current predictor set
        logger.info(f'Predictor set: {predictor_set}')
        coxph = CoxPH(data_dir, results_dir, split_times, seed_to_split, predictor_set, num_workers, logger)
