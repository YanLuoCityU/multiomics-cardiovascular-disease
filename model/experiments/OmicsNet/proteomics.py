...
with open('/your path/multiomics-cardiovascular-disease/config/OmicsNet/Proteomics.json', 'r') as file:
...
for fold in range(10):
    ...
    proteomics_input_dim = data_loader.get_feature(fold=fold, outcomes_list=config['outcomes_list'], predictor='Proteomics').shape[1]
    proteomics_features = data_loader.get_feature_names(fold=fold, outcomes_list=config['outcomes_list'], predictor='Proteomics')
    ...
    train_scores_filename = os.path.join(config['results_dir'], f"ProScore/train_scores_K{fold}.csv")
    ...
    test_scores_filename = os.path.join(config['results_dir'], f"ProScore/test_scores_K{fold}.csv")
    os.makedirs(os.path.dirname(test_scores_filename), exist_ok=True)
    test_scores.to_csv(test_scores_filename, index=False)

    # SHAP value computation
    background = generate_background_samples(train_dataloader, 'Proteomics', num_samples=1000)
    test_batch = next(iter(test_dataloader))
    _, _, _, _, proteomics_data, _, _ = test_batch
    X_test = proteomics_data.to('cpu').float()
    X_test_df = pd.DataFrame(X_test.numpy(), columns=proteomics_features)
    ...
    for outcome in config['outcomes_list']:
        ...
        shap_values_df = pd.DataFrame(shap_values.squeeze(axis=-1), columns=proteomics_features)
